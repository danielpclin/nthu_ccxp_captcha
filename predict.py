...
def predict_ccxp(batch_size=500):
    predict_dir = "dataset/ccxp/validate"
    predict_df = get_dataset_dataframe(predict_dir, out=4)
    model_file = f'checkpoints/ccxp/26.hdf5'
    img_width, img_height = 104, 32
    # img_width, img_height = 52, 16
    datagen = ImageDataGenerator(rescale=1. / 255)
    # predict_generator = datagen.flow_from_dataframe(dataframe=predict_df, directory='.', x_col="file",
    #                                                 y_col=[f'label_{i}' for i in range(1, 5)],
    #                                                 class_mode="multi_output",
    ...
    for digit in pred:
        for index, code in enumerate(digit):
            result[index] = result[index] + int_to_char[code]
    predict_df['pred'] = result
    predict_df.to_csv(f'predict_ccxp.csv', index=False, columns=['file', 'label', 'pred'])
    predict_df.loc[predict_df['label'] != predict_df['pred']].to_csv(f'predict_ccxp_diff.csv', index=False, columns=['file', 'label', 'pred'])
